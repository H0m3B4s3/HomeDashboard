#!/usr/bin/env python3
"""
Production script to import events from a specific iCloud calendar into the HomeBase database.
Designed to run on Raspberry Pi in production environment.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/home/pi/HomeBase/import_events.log'),
...
async def import_events_from_calendar():
    ...
    logger.info(f"📅 Source: {CALENDAR_URL}")
    ...
    async for db in get_db():
        try:
            ...
            if not target_calendar:
                logger.warning("⚠️  No calendar found in database, creating one...")
            ...
            logger.info(f"📋 Target calendar: {target_calendar.name} (ID: {target_calendar.id})")
            ...
            for component in cal.walk():
                if component.name == "VEVENT":
                    ...
                    if existing_event:
                        logger.info(f"⏭️  Skipping existing event: {component.get('summary', 'Unknown')}")
                        events_skipped += 1
                        continue
                    ...
                    if not start_time or not end_time:
                        logger.warning(f"⚠️  Skipping event without start/end time: {component.get('summary', 'Unknown')}")
                        events_skipped += 1
                        continue
                    ...
                    if not end_time or end_time <= now:
                        logger.info(f"⏭️  Skipping past event: {component.get('summary', 'Unknown')} (ended {end_time})")
                        events_skipped += 1
                        continue
                    ...
                    db.add(new_event)
                    events_added += 1
                    logger.info(f"✅ Added: {new_event.title} ({start_time.strftime('%Y-%m-%d %H:%M')})")
            ...
            logger.info(f"⏭️  Events skipped (already exist): {events_skipped}")
            logger.info(f"📅 Total events in database: {events_added + events_skipped}")
...
if __name__ == "__main__":
    logger.info("🚀 HomeBase Calendar Import Tool (Production)")
    logger.info("=" * 50)
